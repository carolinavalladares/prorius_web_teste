...
const UserCard = ({ user, getDeletedUsers }: IProps) => {
  ...
  return (
    <div className="bg-white shadow-sm p-4 rounded-sm relative">
      <div>
        <p className="font-semibold leading-none mb-1 capitalize text-sm">
          {name}
        </p>
        <p className="text-xs sm:text-sm leading-none text-gray-500">{email}</p>
        ...
        {user.deleted ? (
          ...
          <Link title="Mais detalhes" to={`/user/${id}`}>
            <button className="ml-auto mr-0 flex text-xs sm:text-sm items-center justify-between gap-1 transition-all hover:gap-2 hover:pr-1">
              Mais detalhes
      ...
      {role == "ADMIN" ? (
        <span className="absolute top-2 right-2 bg-amber-500 text-white text-[9px] sm:text-[10px] font-semibold px-2 py-1">
          {role}
        </span>
      ) : null}
    </div>
